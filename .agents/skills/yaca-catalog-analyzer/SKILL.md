...
- **Ottimizzazione Payload in `StremioFormatter.js`**: I cataloghi vengono salvati in cache su MongoDB (L2). **Non usare MAI** lo spread operator `...item` in `sanitizeCatalogMeta`, altrimenti ingolferai il DB con i dati raw di TMDB e farai esplodere la cache. I campi pesanti (es. `videos`, `links`, `trailers`) devono essere passati esplicitamente SOLO quando `options.isMetaDetail === true`.
