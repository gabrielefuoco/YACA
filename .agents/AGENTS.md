...
- **Filtri SQL Nativi (No Stringhe TMDB)**: Quando generi filtri complessi in `catalogStrategies.js`, non costruire stringhe "stile TMDB" (es. `with_keywords="1|2"`) per poi passarle ad adapter intermedi (es. `legacyTmdbAdapter.js`). Costruisci programmaticamente la query nativa per DuckDB passando un array `preset.where` popolato con gli helper di `src/data/filters.js` (es. `F.genre()`, `F.keyword()`, `F.any()`). L'array verrà concatenato in `AND` globale da `queryBuilder.js`, garantendo precisione intersezionale assoluta.
