...
def main():
    ...
    # 4. Hub-Penalty per Keyword Umorali (Masking Bipartito)
    # Troviamo gli indici delle keyword umorali
    ...
    if mood_indices:
        ...
        # Maschera: True se entrambi i nodi sono umorali
        mood_to_mood_mask = is_mood[cx_fused.row] & is_mood[cx_fused.col]
        # Azzeriamo gli edge tra mood e mood
        cx_fused.data[mood_to_mood_mask] = 0
        Fused_matrix = cx_fused.tocsr()
        Fused_matrix.eliminate_zeros()
        print(f"   [OK] Applicato Hub-Penalty su {len(mood_indices)} keyword umorali (rimossi edge mood-to-mood).")
