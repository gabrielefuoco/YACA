...
def main():
    print("[START] Inizio Fase 1: Re-build Graph con Livelli L1, L2, L3 (new), L4 (old L3)")
    ...
    # Estrazione Adiacenza solo per SFW
    ...
    # Mapping per saltare buchi nei cluster SFW per igraph
    ...
    partition_L3 = leidenalg.find_partition(ig_G_L2, leidenalg.RBConfigurationVertexPartition, weights=list(l2_adj.values()), resolution_parameter=1.5)
    clusters_L3 = list(partition_L3)
    print(f"   [OK] L3: {len(clusters_L3)} Intermedi")
    ...
    partition_L4 = leidenalg.find_partition(ig_G_L3, leidenalg.RBConfigurationVertexPartition, weights=list(l3_adj.values()), resolution_parameter=1.0)
    clusters_L4 = list(partition_L4)
    print(f"   [OK] L4: {len(clusters_L4)} Macro-Vibes")
    ...
    # 9. Costruzione JSON con Medoidi
    kw_to_L1 = {id_to_kw[i]: f"c_{id_to_cluster[i]}" for i in range(N)}
    
    L1_dict = {}
    for c_id, nodes in enumerate(clusters):
        # We include NSFW nodes in L1 but they have no parent (orphan)
    ...
    print("[DONE] Grafo ricostruito con Medoidi Geometrici!")
